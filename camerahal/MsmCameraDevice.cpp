...
#define ALOG_TAG "MsmCameraDevice"
...
status_t MsmCameraDevice::connectDevice()
{
    ...
    if (!isInitialized()) {
        ALOGE("%s(): Msm camera device is not initialized.", __FUNCTION__);
        return EINVAL;
    }
    if (isConnected()) {
        ALOGW("%s(): Msm camera device is already connected.", __FUNCTION__);
        return NO_ERROR;
    }
    ...
    if (mControlFd < 0) {
        ALOGE("Unable to open the control endpoint [ERR %s]", 
             strerror(errno));
        return ALREADY_EXISTS;
    }
    mConfigFd = openEndpoint(MSM_CONFIG);
    if (mConfigFd < 0) {
        ALOGE("Unable to open the config endpoint [ERR %s]",
             strerror(errno));
        close(mControlFd);
        return ALREADY_EXISTS;
    }
    ...
    if (!getVendorId()) {
        ALOGE("Unable to communicate with the device.");
        close(mConfigFd);
        close(mControlFd);
        return BAD_VALUE;
    }
    ...
    if (getConfigThread() == NULL) {
        ALOGE("%s: Unable to instantiate config thread object", __FUNCTION__);
        close(mConfigFd);
        close(mControlFd);
        return ENOMEM;
    }
...
status_t MsmCameraDevice::startDevice(int width, int height, uint32_t pix_fmt)
{
    ...
    if (res != NO_ERROR) {
        ALOGE("Unable to start device...");
        return res;
    }
    res = getConfigThread()->startThread();
    ALOGE_IF(res != NO_ERROR, "%s: Unable to start config thread", __FUNCTION__);
...
bool MsmCameraDevice::configCommand(int cmd, void *ptr)
{
    if (__ioctl(IOCTL_CONFIG, cmd, ptr))
        return true;       
    ALOGE("configCommand failed. Error: %s", strerror(errno));
    return false;
}
...
int MsmCameraDevice::openEndpoint(msm_endpoint_e which)
{
    ...
    if (fd < 0)
        ALOGE("Error opening %s, %s", endpoint, strerror(errno));
    return fd;
}
...
bool MsmCameraDevice::getVendorId()
{
    ...
    if (configIoctl(MSM_CAM_IOCTL_SENSOR_IO_CFG, &cfg) == false) {
        ALOGE("Failed to get I2C sensor data.");
        return false;
    }

    struct fuse_id *f = &cfg.cfg.fuse;
    if (f->fuse_id_word1 != 0 || f->fuse_id_word2 != 0) {
        ALOGE("Read I2C data, but data appears invalid.");
        return false;
    }   
/*
    ALOGD("Got sensor IO data...");
    ALOGD("    fuse #1: %08x", f->fuse_id_word1);
    ALOGD("    fuse #2: %08x", f->fuse_id_word2);
    ALOGD("    fuse #3: %08x", f->fuse_id_word3);
    ALOGD("    fuse #4: %08x", f->fuse_id_word4);
*/
    mVendorId = f->fuse_id_word3;
    ALOGD("Sensor Vendor ID: %d", mVendorId);
    return true;
}
...
bool MsmCameraDevice::setSensorMode(int mode)
{
    ...
    if (configIoctl(MSM_CAM_IOCTL_SENSOR_IO_CFG, &cfg)) {
        ALOGD("Set sensor mode to %d", mode);
        return true;
    }
    ALOGE("Error setting sensor mode to %d", mode);
    return false;
}    
...
bool MsmCameraDevice::getSensorInformation()
{
    ...
    if (configIoctl(MSM_CAM_IOCTL_GET_SENSOR_INFO, &mSensorInfo))
        ALOGD("Sensor information:: name: %s, flash %d, steps %d", 
    ...
    if (configIoctl(MSM_CAM_IOCTL_SENSOR_IO_CFG, &cfg))
        ALOGD("Picture FPS: %d", cfg.cfg.gfps.pictfps);
    
    memset(&cfg, 0, sizeof(cfg));
    cfg.cfgtype = CFG_GET_PREV_L_PF;
    if (configIoctl(MSM_CAM_IOCTL_SENSOR_IO_CFG, &cfg))
        ALOGD("Preview LPF: %d", cfg.cfg.prevl_pf);

    memset(&cfg, 0, sizeof(cfg));
    cfg.cfgtype = CFG_GET_PREV_P_PL;
    if (configIoctl(MSM_CAM_IOCTL_SENSOR_IO_CFG, &cfg))
        ALOGD("Preview PPL: %d", cfg.cfg.prevp_pl);
   
//    ALOGE("Failed to get sensor information. Error: %s", strerror(errno));
    return true;
}
...
*/
    ...
    ALOGD("%s: nfunctions = %d, so %d blocks", __FUNCTION__, nTypes, nBlocks);
    ...
    if (! mSetupMemory->initialized()) {
        ALOGE("%s: failed to get pmem block...", __FUNCTION__);
        return false;
    }
    ...
    for (int i = 0; i < nTypes; i++) {
        ...
        for (int j = 0; j < NUM_STAT_OUTPUT_BUFFERS; j++) {
            ...
            if (ck == false) {
                ALOGE("Unable to set memory for %08x:%d", types[i], j);
                break;
            }
            ALOGD("Set block %08x:%d OK", types[i], j);
        }
    }
    return ck;
}
...
bool MsmCameraDevice::__ioctl(int which, int cmd, void *ptr)
{
    ...
    if (fd == -1) {
        ALOGE("%s:: %s endpoint is NOT open", __FUNCTION__, 
             (which == IOCTL_CONFIG ? "config" : "control"));
        return false;
    }
    if (ioctl(fd, cmd, ptr) == 0)
        return true;
    ALOGE("%s:: failed. cmd = %d, error = %s", __FUNCTION__, _IOC_NR(cmd),
         strerror(errno));
    return false;
}
...
bool MsmCameraDevice::controlCommand(uint16_t type, uint16_t length, void *value)
{
    ...
    ALOGD("issuing control command");
    bool rv = controlIoctl(MSM_CAM_IOCTL_CTRL_COMMAND, &cmd);
    if (false == rv || 0== cmd.status) {
        ALOGE("%s(%d) failed.", __FUNCTION__, type);
        return false;
    }
    ALOGD("%s(%d) succeeded", __FUNCTION__, type);
    return true;
}
    ...
    while (true) {
        ...
        switch (evt.resptype) {
            case MSM_CAM_RESP_CTRL:
                 ...
                 */
                ALOGD("    ctrl_cmd:");
                ALOGD("        type:      %d", evt.ctrl_cmd.type);
                ALOGD("        length:    %d", evt.ctrl_cmd.length);
                ALOGD("        value:     %p", evt.ctrl_cmd.value);
                for (int i = 0; i < evt.ctrl_cmd.length; i++)
                    ALOGD("        buffer %d:   %02x", i, buffer[i]);
                ALOGD("        resp_fd:   %d", evt.ctrl_cmd.resp_fd);
                ALOGD("        status:    %d", evt.ctrl_cmd.status);
            ...
            case MSM_CAM_RESP_STAT_EVT_MSG:
                ...
                ALOGD("ConfigThread: Received an MSM_CAM_RESP_STAT_EVT_MSG");
                break;
            case MSM_CAM_RESP_V4L2:
                /* Video 4 Linux */
                ALOGD("ConfigThread: Received an MSM_CAM_RESP_V4L2");
                break;
                
        }
        if (false == rv) {
            ALOGE("Error responding to dequeued command, aborting");
            return false;
        }
    }
    return false;
...
bool MsmCameraDevice::disableStats()
{
    ...
    bool rc = configIoctl(MSM_CAM_IOCTL_CONFIG_VFE, &vfe);
    ALOGE_IF(!rc, "Failed to disable STATS");
    ALOGD_IF(rc, "STATS disabled");
    return rc;
}
...
bool MsmCameraDevice::enableAF()
{
    ...
    bool rc = configIoctl(MSM_CAM_IOCTL_CONFIG_VFE, &vfe);
    ALOGE_IF(!rc, "Failed to enable AF STATS");
    ALOGD_IF(rc, "AF STATS enabled");
    return rc;
}
...
bool MsmCameraDevice::enableAWB()
{
    ...
    bool rc = configIoctl(MSM_CAM_IOCTL_CONFIG_VFE, &vfe);
    ALOGE_IF(!rc, "Failed to enable AWB STATS");
    ALOGD_IF(rc, "AWB STATS enabled");
    return rc;
}
...
bool MsmCameraDevice::enableAEC()
{
    ...
    bool rc = configIoctl(MSM_CAM_IOCTL_CONFIG_VFE, &vfe);
    ALOGE_IF(!rc, "Failed to enable AEC STATS");
    ALOGD_IF(rc, "AEC STATS enabled");
    return rc;
}
...
status_t MsmCameraDevice::ConfigThread::readyToRun()
{
    ...
    if (NULL == mCameraDevice) {
        ALOGV("Unable to start config thread as no MsmCameraDevice");
        return BAD_VALUE;
    }
    if (-1 == mCameraDevice->mConfigFd || -1 == mCameraDevice->mControlFd) {
        ALOGE("Unable to start config thread as endpoints not open");
        return BAD_VALUE;
    }
    return NO_ERROR;
}
...
{
    ALOGV("Stopping  camera device's worker thread...");
