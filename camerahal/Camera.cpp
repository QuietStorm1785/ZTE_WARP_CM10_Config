...
char* Camera::getParameters()
{
    ...
    if (NULL == params_string) {
        ALOGE("%s(): Unable to allocate memory for return string.", 
             __FUNCTION__);
        return &lNoParam;
    }
...
int Camera::setParameters(const CameraParameters& params)
{
    ...
    if (! isPreviewEnabled()) {
        ...
        for (int i = 0; i < nkeys ; i++) {
            ...
            ALOGD("Key %d: %s => %s", i, keys[i], valstr);
            if (valstr != NULL)
...
{
    ALOGD("%s() -> %d", __FUNCTION__, mPreviewWindow.isPreviewEnabled());
...
{
    ALOGD("%s(%d) - not implemented yet ", __FUNCTION__, fd);
    return NO_ERROR;
}

status_t Camera::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ALOGV("%s(%d, %d, %d) - not implemented yet ", __FUNCTION__, cmd, arg1, arg2);
    return NO_ERROR;
}
